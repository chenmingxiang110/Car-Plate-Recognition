...
class ctc_recog_model:
    ...
    def __init__(self, num_class, topk_paths = 10):
        ...
        conv1 = self._nn_conv_bn_layer(self.xs, 'conv_1', [11, 41, 3, 16], [3, 2])
        conv2 = self._nn_conv_bn_layer(conv1, 'conv_2', [11, 21, 16, 16], [2, 2])
        conv_out = tf.reshape(conv2, [-1, 50, 38*16])
        biRNN1 = self._biRNN_bn_layer(conv_out, 'biRNN_1', 128)
        biRNN2 = self._biRNN_bn_layer(biRNN1, 'biRNN_2', 128)

        self.phonemes = tf.layers.dense(biRNN2, num_class)
    ...
    def train(self, sess, learning_rate, xs, ys):
        _, loss, summary = sess.run([self.train_op, self.loss, self.merged], feed_dict = {self.isTrain: True, self.learning_rate: learning_rate, self.seq_len: np.ones(xs.shape[0])*50, self.xs: xs, self.ys: ys})
        return loss, summary

    def get_loss(self, sess, xs, ys):
        loss = sess.run(self.loss, feed_dict = {self.isTrain: False, self.seq_len: np.ones(xs.shape[0])*50, self.xs: xs, self.ys: ys})
        return loss

    # return top k results in sparse matrix
    def predict(self, sess, xs):
        prediction = sess.run(self.prediction, feed_dict = {self.isTrain: False, self.seq_len: np.ones(xs.shape[0])*50, self.xs: xs})
        return prediction
