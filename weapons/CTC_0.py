...
import numpy as np
from weapons.layerNormedGRU import layerNormedGRU

class ctc_recog_model:

    # num_class = actual num of classes + 2
    def __init__(self, num_class, topk_paths = 10):
        self.xs = tf.placeholder(tf.float32, [None, 300, 150, 3])
        self.ys = tf.sparse_placeholder(tf.int32)
        self.learning_rate = tf.placeholder(tf.float32)
        self.seq_len = tf.placeholder(tf.int32, [None])
        self.isTrain = tf.placeholder(tf.bool, name='phase')

        conv1 = self._nn_conv_bn_layer(self.xs, 'conv_1', [11, 41, 3, 16], [2, 2])
        conv2 = self._nn_conv_bn_layer(conv1, 'conv_2', [11, 21, 16, 32], [1, 2])
        conv_out = tf.reshape(conv2, [-1, 150, 38*32])
        biRNN1 = self._biRNN_bn_layer(conv_out, 'biRNN_1', 128)
        biRNN2 = self._biRNN_bn_layer(biRNN1, 'biRNN_2', 128)
        biRNN3 = self._biRNN_bn_layer(biRNN2, 'biRNN_3', 128)
    ...
    def _nn_conv_bn_layer(self, inputs, scope, shape, strides):
        with tf.variable_scope(scope):
            ...
            h_bn = tf.layers.batch_normalization(h_conv+b, training = self.isTrain, momentum = 0.7)
    ...
    def train(self, sess, learning_rate, xs, ys):
        _, loss, summary = sess.run([self.train_op, self.loss, self.merged], feed_dict = {self.isTrain: True, self.learning_rate: learning_rate, self.seq_len: np.ones(xs.shape[0])*150, self.xs: xs, self.ys: ys})
        return loss, summary

    def get_loss(self, sess, xs, ys):
        loss = sess.run(self.loss, feed_dict = {self.isTrain: False, self.seq_len: np.ones(xs.shape[0])*150, self.xs: xs, self.ys: ys})
        return loss

    # return top k results in sparse matrix
    def predict(self, sess, xs):
        prediction = sess.run(self.prediction, feed_dict = {self.isTrain: False, self.seq_len: np.ones(xs.shape[0])*150, self.xs: xs})
        return prediction
