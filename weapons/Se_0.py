...
class seg_model:
    ...
    def __init__(self):
        ...
        with tf.variable_scope("block1"):
            ...
            conv23 = self._nn_conv_bn_layer(conv22, 'conv_23', [3, 3, 64, 64], [1, 1])
            conv24 = self._nn_conv_bn_layer(conv23, 'conv_24', [3, 3, 64, 64], [1, 1])
            conv24 = tf.add(conv22, conv24)
            conv24_shortcut = self._nn_conv_bn_layer(conv24, 'conv_24s', [3, 3, 64, 128], [2, 2])
        with tf.variable_scope("block2"):
            conv31 = self._nn_conv_bn_layer(conv24, 'conv_31', [3, 3, 64, 128], [2, 2])
            conv32 = self._nn_conv_bn_layer(conv31, 'conv_32', [3, 3, 128, 128], [1, 1])
            conv32 = tf.add(conv24_shortcut, conv32)
        ...
        with tf.variable_scope("block3"):
            ...
            conv43 = self._nn_conv_bn_layer(conv42, 'conv_43', [3, 3, 256, 256], [1, 1])
            conv44 = self._nn_conv_bn_layer(conv43, 'conv_44', [3, 3, 256, 256], [1, 1])
            conv44 = tf.add(conv42, conv44)
        with tf.variable_scope("block4"):
            conv5 = self._nn_atrous_conv_bn_layer(conv44, 'conv_5', [3, 3, 256, 512], 2)
